...
## Android 1.0.11 系统导航适配

- Android 11 及以上由原生容器统一避开状态栏、导航栏、屏幕缺口和键盘；三键、手势导航及横竖屏切换都会重新计算可用区域。Android 8–10 保留系统默认的窗口适配。
- 安全区域在原生层处理并消费，避免网页再次添加同样的留白；网页弹窗、底栏和悬浮按钮都在缩小后的可用区域内显示。
- 更新需要安装新版 APK；仅刷新网页不能更新原生窗口行为。
